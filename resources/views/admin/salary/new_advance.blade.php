    ...
    <section>
        <div class="card">
            <div class="card-body">
                ...
                <div class="alert alert-callout alert-success "><b class="text-lg">অগ্রিম</b>
                </div>
                <div class="">
                    <form action="{{ url('/admin/new_advance') }}" method="post">
                        {{ csrf_field() }}
                                <!---date--->
                        <div class="row">
                            <div class="col-sm-8 col-md-offset-2">
                                        ...
                                        <select required class="form-control" name="employeeId" id="employeeId">
                                            ...
                                            @else
                                                @foreach($employeeId as $row)
                                                    <option @if(old('employeeId')==$row->id){{'selected'}}@endif value="{{ $row->id }}">{{ $row->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                ...
                                <div class="col-sm-12 form-group {{ $errors->has('month')?'has-error':'' }}">
                                    ...
                                    <div class="col-sm-9">
                                        <input readonly type="text" name="month" id="month" class="form-control"
                                               required
                                               value="@if(old('month')){{old('month')}}@else{{date('Y-m')}}@endif">
                                ...
                                <div class="col-sm-12 form-group {{ $errors->has('dateReceived')?'has-error':'' }}">
                                    <label for="dateReceived" class="col-sm-3"><span
                                                style="color: red; font-size: 16px;">* </span>অগ্রীম গ্রহণের
                                        তারিখ</label>
                                    <div class="col-sm-9">
                                        <input readonly type="text" name="dateReceived" id="dateReceived"
                                               class="form-control dateInput" required
                                               value="@if(old('dateReceived')){{old('dateReceived')}}@else{{date('Y-m-d')}}@endif">
